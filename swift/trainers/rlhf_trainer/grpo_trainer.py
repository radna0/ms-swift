...
try:
    ...
    raise ImportError(
        "Please install trl from source using: `pip install git+https://github.com/huggingface/trl.git`"
    )
...
class GRPOCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        train_dataloader = getattr(state, "train_dataloader", None) or kwargs.get(
            "train_dataloader"
        )
...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.args.tensor_parallel_size > 1:
            assert (
                get_device_count() == local_world_size == self.args.num_infer_workers
                and local_world_size > 1
            ), (
                "tensor_parallel_size>1 only supports num_infer_workers==your "
                "available device count."
            )
        if self.args.async_generate:
            assert (
                local_world_size + self.args.num_infer_workers <= get_device_count()
            ), ("async_generate requires training and rollout use " "different GPUS.")

        if self.args.sleep_level > 0:
            if local_world_size + self.args.num_infer_workers <= get_device_count():
                logger.warning(
                    "You are using different GPUs for training and rollout, "
        ...
        if reward_funcs:
            for i, reward_func in enumerate(reward_funcs):
                if reward_func in orms:
                    ...
                    reward_func_args = list(
                        inspect.signature(reward_func_class.__init__).parameters
                    )
                    reward_func_kwargs = {
                    ...
                    raise ValueError(
                        f"reward_function {reward_func} is not implemented in swift.llm.plugin"
                    )
            ...
            if self.infer_rank >= 0:
                ...
                if fast_infer_device[0] == "auto":
                    if get_device_count() == 1:
                        fast_infer_device = [
                            get_device()
                        ]  # particular case when training with only 1 GPU: share it
                    else:
                        fast_infer_device = []
                        for idx in range(
                            get_device_count() - self.args.num_infer_workers,
                            get_device_count(),
                        ):
                ...
                for _device in fast_infer_device:
                    ...
                    if (
                        _device.split(":")[0] in {"cuda", "npu"}
                        and int(_device.split(":")[1]) >= get_device_count()
                    ):
                        raise ValueError(
                            f"The requested device for vllm ({_device}) is not available. "
                    ...
                    if _device in {
                        get_device(idx) for idx in range(self.accelerator.num_processes)
                    }:
                        logger.warning(
                            f"The requested device {_device} is also used for training. "
                ...
                if use_vllm:
                    ...
                    if not is_lmdeploy_available():
                        raise ImportError(
                            "LMDeploy is not available and `use_lmdeploy` is set to True."
                    ...
                    with Swift.grpo_context(model, self.template.processor):
                        fast_infer_device = int(
                            fast_infer_device[self.local_infer_rank].split(":")[1]
                        )
        ...
        for param_group in self.optimizer.param_groups:
            for param in param_group["params"]:
                ...
                for key, value in state.items():
                    if isinstance(value, torch.Tensor):
                        state[key] = value.to(
                            self.offload_states[key], non_blocking=True
                        )
    ...
    def _move_model_to_vllm_lmdeploy(self):
        ...
        for i, parameter_group in enumerate(self.parameter_groups):
            ...
            with unwrap_model_for_generation(
                ...
                gather_parameters=parameter_group,
            ) as unwrapped_model:
                ...
                def merge(
                    ...
                    if parameter_group and all(
                        [self.name not in pg for pg in parameter_group]
                    ):
                        # Not this group, skip
                        return
                    else:
                ...
                def get_delta_weight(self, adapter) -> torch.Tensor:
                    # may be offload
                    self.lora_A[adapter].weight.data = self.lora_A[
                        adapter
                    ].weight.data.to(self.base_layer.weight.device)
                    self.lora_B[adapter].weight.data = self.lora_B[
                        adapter
                    ].weight.data.to(self.base_layer.weight.device)
                ...
                def patch_merge(model):
                    ...
                    for name, module in model.named_modules():
                        if isinstance(module, LoraLayer):
                            module.name = name
                            if not hasattr(module, "merge_origin") and hasattr(
                                module, "base_layer"
                            ):
                                ...
                                module.get_delta_weight = MethodType(
                                    get_delta_weight, module
                                )
                    yield
                    for name, module in model.named_modules():
                        if isinstance(module, LoraLayer):
                ...
                if parameter_group_no_lora:
                    parameter_group_no_lora = [
                        n.replace("base_model.model.", "")
                        for n in parameter_group_no_lora
                    ]
                    state_dict = {
                        k: v
                        for k, v in state_dict.items()
                        if k in parameter_group_no_lora
                    }
                assert all(
    ...
    def _prefetch(self, dataloader):
        ...
        distributed_idx = self.round_robin(
            len(all_inputs), get_node_setting()[1] * self.args.num_infer_workers
        )
        if self.infer_rank >= 0:
            _input_slice = np.array(all_inputs)[distributed_idx[self.infer_rank]]
            outputs = self.engine.infer(
                _input_slice, self.request_config, use_tqdm=False
            )
    ...
    def _generate_and_score_completions(
        ...
        with self._template_context(template):
            batched_inputs = [
                template.encode(infer_request) for infer_request in inputs
            ]
            outputs = to_device(
                template.data_collator(batched_inputs), self.model.device
            )

        # we only need to compute the logits for the completion tokens
        labels = outputs.pop("labels")
        logits_to_keep = (
            (labels.shape[-1] - (torch.ne(labels, -100).int().argmax(-1))).max().item()
        )
        ...
        with torch.no_grad():
            if self.old_policy:
                outputs["old_per_token_logps"] = self._get_per_token_logps(
                    self.model, outputs
                )
            else:
        ...
        rewards_per_func = torch.zeros(
            (len(inputs), len(self.reward_funcs)), device=device
        )
        completions = [example["messages"][-1]["content"] for example in inputs]

        for i, (reward_func, reward_template) in enumerate(
            zip(self.reward_funcs, self.reward_templates)
        ):
            if isinstance(
                ...
                with self._template_context(reward_template):
                    batched_inputs = [
                        reward_template.encode(infer_request)
                        for infer_request in inputs
                    ]
                    reward_inputs = to_device(
                        reward_template.data_collator(batched_inputs),
                        reward_func.device,
                    )
        ...
        if (
            ...
            table = {
                ...
                "messages": [
                    inputs["messages"][:-1] for inputs in gather_object(inputs)
                ],
    ...
    def _get_per_token_logps(self, model, inputs):
        ...
        if (
            not unwrapped_model.model_meta.is_multimodal
            and "logits_to_keep" in parameters
        ):
            # save memory
            return super()._get_per_token_logps(
                model, input_ids, inputs["attention_mask"], logits_to_keep
            )
        inputs = {
            k: v
    ...
    def evaluation_loop(self, dataloader, *args, **kwargs):
        ...
        metrics = {
            f"{metric_key_prefix}_{key}": sum(val) / len(val)
            for key, val in self._metrics["eval"].items()
        }
