...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _prepare_generation_config(self):
        ...
        self.model.generation_config = prepare_generation_config(
            self.model.generation_config, args.get_request_config(), self.tokenizer
        )
    ...
    def _get_dataset(self):
        ...
        if len(args.val_dataset) > 0:
            ...
            _, val_dataset = load_dataset(
                args.val_dataset, split_dataset_ratio=1.0, **dataset_kwargs
            )
            assert args.split_dataset_ratio == 0.0
        logger.info(f"train_dataset: {train_dataset}")
        logger.info(f"train_dataset[0]: {train_dataset[0]}")
        logger.info(f"val_dataset: {val_dataset}")
        logger.info(f"val_dataset[0]: {val_dataset[0]}")
    ...
    def run(self):
        ...
        if isinstance(val_dataset, HfDataset):
            ...
            logger.info(
                f"The split dataset from the training set will be saved at: {val_dataset_path}."
            )
        if (
            args.task_type == "seq_cls"
            and isinstance(train_dataset, HfDataset)
            and "label" in train_dataset.features
        ):
            min_num_labels = int(max(train_dataset["label"]) + 1)
            assert (
                args.num_labels >= min_num_labels
    ...
    def _prepare_callbacks(self):
        ...
        if args.lisa_activated_layers > 0:
            ...
            ), "LISA only supports full parameter training."
    ...
    def _encode_dataset(self, train_dataset, val_dataset):
        ...
        if not is_grpo:
            if args.lazy_tokenize:
                ...
                preprocessor_cls = (
                    PackingPreprocessor if args.packing else EncodePreprocessor
                )
                preprocessor = preprocessor_cls(template=template)
                train_dataset = preprocessor(
                    train_dataset, num_proc=args.dataset_num_proc, strict=args.strict
                )
                if val_dataset is not None and not args.predict_with_generate:
                    val_dataset = preprocessor(
                        val_dataset, num_proc=args.dataset_num_proc, strict=args.strict
                    )

            inputs = (
                train_dataset[0]
                if hasattr(train_dataset, "__len__")
                else next(iter(train_dataset))
            )
            template.print_inputs(
                inputs, tokenizer_kwargs=inputs.pop("tokenizer_kwargs", None) or {}
            )
