...
class DeepseekTemplateMeta(TemplateMeta):
    ...
    system_prefix: Optional[Prompt] = field(
        default_factory=lambda: [["bos_token_id"], "{{SYSTEM}}\n\n"]
    )
...
class DeepseekVLTemplate(Template):
        ...
        if not inputs.generate_mode:  # understanding task
            ...
            output = VLChatProcessorOutput(
                ...
                num_image_tokens=torch.tensor(
                    [processor.num_image_tokens] * len(idx_list)
                ),
            )
            encoded = {
                "output": output,
            ...
            if self.is_training:
                raise NotImplementedError(
                    "Only support the inference of generation of Janus series models."
                )
    ...
    def _data_collator(
        ...
        if all(gene_img_list):
            ...
            raise NotImplementedError(
                "Do not support understanding and image generation tasks in one batch."
            )
    ...
    def generate(self, model, *args, **kwargs):
        if not kwargs.get("generate_mode"):
            ...
            tokens = torch.zeros(
                (bsz, parallel_size * 2, max_input_token_num), dtype=torch.int
            ).cuda()  # [bsz, parallel_size*2, max_input_token_num]
            ...
            inputs_embeds = model.language_model.get_input_embeddings()(
                tokens
            )  # [bsz, parallel_size*2, max_input_token_num, 2048]

            generated_tokens = torch.zeros(
                (bsz, parallel_size, self.image_token_num_per_image), dtype=torch.int
            ).cuda()  # [bsz, 16, image_token_num_per_image] placeholder for the generated tokens

            # set the first two dimensions into one dimension for batch size
            inputs_embeds = inputs_embeds.reshape(
                bsz * parallel_size * 2, max_input_token_num, -1
            )
            ...
            for i in range(
                self.image_token_num_per_image
            ):  # generate the tokens of image in a auto-regression way
                ...
                img_embeds = model.prepare_gen_img_embeds(
                    next_token
                )  # [parallel_size * 2, 2048]
                inputs_embeds = img_embeds.unsqueeze(
                    dim=1
                )  # [parallel_size * 2, 1, 2048]
    ...
    def decode(self, generate_ids: List[int], **kwargs) -> Any:
        if (
            "template_inputs" not in kwargs
            or not kwargs["template_inputs"].generate_mode
        ):
            ...
            generate_ids = torch.tensor(generate_ids).unsqueeze(
                0
            )  # [num_to_decode=1, self.image_token_num_per_image]
            ...
            dec = (
                dec.to(torch.float32).cpu().numpy().transpose(0, 2, 3, 1)
            )  # [num_to_decode, H, W, ch=3]

            dec = np.clip((dec + 1) / 2 * 255, 0, 255)

            visual_img = np.zeros(
                (num_to_decode, img_size, img_size, 3), dtype=np.uint8
            )
            visual_img[:, :, :] = dec
...
class DeepseekVLTemplateMeta(DeepseekTemplateMeta):
    ...
    placeholder_tokens: List[str] = field(
        default_factory=lambda: ["<image_placeholder>"]
    )
...
register_template(
    DeepseekVLTemplateMeta(MLLMTemplateType.deepseek_janus, template_cls=DeepseekJanus)
)
...
class DeepseekV2_5TemplateMeta(TemplateMeta):
    prefix: Prompt = field(
        default_factory=lambda: ["<｜begin▁of▁sentence｜>{{SYSTEM}}"]
    )
    prompt: Prompt = field(
        default_factory=lambda: ["<｜User｜>{{QUERY}}<｜Assistant｜>"]
    )
    chat_sep: Optional[Prompt] = field(
        default_factory=lambda: ["<｜end▁of▁sentence｜>"]
    )
...
register_template(
    DeepseekV2_5TemplateMeta(
        LLMTemplateType.deepseek_r1, template_cls=DeepseekR1Template
    )
)
...
class DeepseekVL2Template(DeepseekVLTemplate):
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        _, images_list, _, images_spatial_crop, num_image_tokens = (
